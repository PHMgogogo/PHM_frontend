        ...
        <template v-if="hasOutParameters">
            ...
            <el-form-item v-for="key in Object.keys(nodeSchema.$defs.OutParameters.properties)" :key="'out_' + key"
                ...
                <el-dropdown @command="(cmd: string) => setOutParam(key, cmd)" style="width: 100%"
                    ...
                    <el-input-tag :model-value="outParamValue(key)"
                        @change="(val: string[]) => setOutParamList(key, val)" />
                    <template #dropdown>
                        <el-dropdown-menu>
...
import { getInstances, getNodeTypes, getNodeSchema } from '@/api/pipelines';
...
const possibleReadData = computed(() => {
    ...
    // read_data 候选：所有 order 小于当前节点的节点的 write_data
    const currentOrder = formData.value?.order ?? -1
    const columns = new Set<string>()
    for (const node of props.graphConfigData.nodes) {
        const data = node?.properties?.data
        if (data && (data.order ?? -1) < currentOrder) {
...
const possibleWriteData = computed(() => {
    if (!props.graphConfigData?.nodes) return []
    // write_data 候选：所有 order 大于当前节点的节点的 read_data
    const currentOrder = formData.value?.order ?? -1
    const columns = new Set<string>()
    for (const node of props.graphConfigData.nodes) {
        const data = node?.properties?.data
...
// out_parameters 单值字段用 el-input-tag 交互：内部仍存单个字符串，通过数组适配
function outParamValue(key: string): string[] {
    const v = formData.value?.out_parameters?.[key]
    return v ? [String(v)] : []
}
function setOutParamList(key: string, val: string[]) {
    if (formData.value?.out_parameters) {
        formData.value.out_parameters[key] = val.length ? val[val.length - 1] : ''
    }
}
const instances = ref<any[]>()
// 只加载属于当前 aircraft_id 的实例：先取该飞机的 instance_id 列表（tasks 表），再过滤全量实例
async function loadInstancesForAircraft(): Promise<any[]> {
    const all = await getInstances()
    if (!props.aircraftNumber) return all ?? []
    try {
        const { tasks } = await taskApi.listByAircraft(props.aircraftNumber)
        const allowed = new Set((tasks ?? []).map(t => t.instance_id).filter(Boolean))
        return (all ?? []).filter(inst => allowed.has(inst.id))
    } catch {
        // 查询失败时回退为全量实例，避免阻塞编辑
        return all ?? []
    }
...
async function updateNode(data: NodeData | null) {
    if (data) {
        ...
        if (["InferenceNode", "TrainNode", "SendAlarmNode"].includes(data.node_type)) {
            instances.value = await loadInstancesForAircraft()
        }
