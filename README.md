...
- **认证**：opencode v2 使用 HTTP Basic，用户名固定 `opencode`。服务端密码经环境变量 `OPENCODE_PASSWORD` 指定（见 `algo/algorithms/opencode/start.bash`），前端须配置同一密码；若服务端未设置该变量，则每次启动随机生成密码，此时需将 `OPENCODE_AUTH.pass` 置空。
- **后端版本**：对接 opencode v2（`opencode2 serve`）。v2 全部接口位于 `/api/*` 前缀下，工作目录改用 `location[directory]` 查询参数传递（旧版 `x-opencode-directory` 请求头已失效），认证仍为 HTTP Basic。
- **REST**：`GET /api/health`（健康检查）、`GET /api/model`（模型列表）、`GET /api/provider`、会话 CRUD（`GET|POST /api/session`、`DELETE /api/session/:id`）、`POST /api/session/:id/prompt`（发消息，回复走 SSE）、`POST /api/session/:id/interrupt`（中止）、`GET|POST /api/session/:id/form[/:formID/reply|/cancel]`（AI 提问交互）。
- **SSE 流式**（`GET /api/event`）：事件业务数据统一放在 `data` 字段，按事件类型分发——
  - `session.text.delta` / `session.reasoning.delta`：增量 token，缓冲后按 `requestAnimationFrame` 每帧合并，按消息聚合，避免逐 token 全量重渲染；`session.text.started/ended`、`session.reasoning.started/ended` 标识分段收尾；
  - `session.execution.started/succeeded/failed`：执行生命周期，`succeeded` 时收尾对齐并重拉消息；
  - `session.step.started/ended`：单步（一次 provider turn）开始 / 结束；
  - `session.tool.*`：工具调用状态，触发消息重拉；
  - `session.form.created/state/replied/cancelled`：AI 主动提问交互（v2 form 取代旧 question）。
